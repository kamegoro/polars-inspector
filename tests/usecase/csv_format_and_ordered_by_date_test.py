...
def test_CSVを読み込んで整形されて日付が降順に並び替えられる(
    mock_read, mock_clean, mock_sortByDate, mock_write
):
    base_dataframe = Dataframe(
        {"name": [" aLICE", "Bob"], "date": ["2021-01-01", "2021-01-02"]}
    )
    cleaned_dataframe = Dataframe(
        {"name": ["ALICE", "BOB"], "date": ["2021-01-01", "2021-01-02"]}
    )
    ordered_dataframe = Dataframe(
        {"name": ["ALICE", "BOB"], "date": ["2021-01-02", "2021-01-01"]}
    )
